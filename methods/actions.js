...
var NotificationActive = require("../models/notificationactive");
...
var DOMAIN =
  "https://api.mailgun.net/v3/sandboxb25e2fc7cf984723b32c0fec7547984e.mailgun.org";
var mg = mailgun({
  apiKey: "3fc8dfdd8323144c34284eb76b5d5ba1-d32d817f-2f5bf026",
  domain: DOMAIN,
});
...
var functions = {
      ...
      User.findOne(
        ...
        function (err, user) {
          ...
          if (!user) {
            ...
            var mailOptions = {
              from: "ManagerPassenger ✔ <xxxx@gmail.com>",
              to: req.body.email,
            ...
            return res
              .status(400)
              .send({ success: false, msg: "Email existed!" });
          }
        }
      );
    }
  ...
  sendorder: function (req, res) {
    ...
    var mailOptions = {
      from: "BusManage ✔ <xxxx@gmail.com>",
      to: req.body.email,
      subject: "App BusManage",
      html: `<h4>Cảm ơn  ${req.body.name} đã đặt vé của hệ thống chúng tôi.</h4>
               <p>Bạn đã đặt vé thành công!</p>
               <p>Hóa đơn của bạn ${req.body.orderID}</p>
               <p>Họ tên:  ${req.body.name}</p>
               <p>Email:  ${req.body.email}</p>
               <p>Số điện thoại:  ${req.body.phone}</p>
               <p>Thời gian:  ${req.body.timetour}</p>
               <p>Địa điểm:  ${req.body.location}</p>
               <p>Ghế:  ${req.body.seat}</p>
               <p>Tổng tiền:  ${req.body.totalprice}</p>`,
    };
    ...
    transporter.sendMail(mailOptions, function (error, info) {
      if (error) {
        ...
        console.log("Email sent: " + info.response);
        res.json({ success: true, msg: "Mail Successfully" });
      }
    });
  },
  ...
  updateinfo: function (req, res) {
    ...
    User.findOneAndUpdate(
      query,
      { name: req.body.name, phone: req.body.phone, image: req.body.image },
      { upsert: true },
      function (err, doc) {
        if (err) return res.send(500, { error: err });
        doc.save(function (err, newPass) {
  ...
  sendNotiGet: function (req, res) {
    console.log(req.query.findByIdAndUpdate);
    var fcm = new FCM(
      "AAAASCcqYL8:APA91bGmLwvGdQER_HJBV0jNBwPB5227We20NoM5Q5Zw5XxFm0nyGUzAOHrDP0jesrJuQoz2S0lHZ3l0fPIpbPt43YiM068wX0J973QFBET_j6Sb2tRVOee7DMuRRbLF498ZfD-i0ajl"
    );
  ...
  getActive: function (req, res) {
    if (req.params.registerID != null) {
      NotificationActive.find(
      ...
      NotificationActive.find({}, function (err, notification) {
        if (err || !notification) {
          res.status(403).send({ success: false, msg: "Not found" });
        } else {
          return res.json(notification);
  ...
  addActive: function (req, res) {
    if (!req.body.registerID || !req.body.activeID || !req.body.tokenID) {
      console.log(req.body);
      res.json({ success: false, msg: "Enter all fields" });
    } else {
      var newNotification = NotificationActive({
  ...
  updateActive: function (req, res) {
    NotificationActive.findOneAndUpdate(
      { registerID: req.body.registerID },
      { activeID: req.body.activeID, tokenID: req.body.tokenID },
  ...
  deleteActive: function (req, res) {
    NotificationActive.findOneAndDelete(
      { registerID: req.body.registerID },
      function (err, address) {
        if (err || !address) {
          res.status(403).send({ success: false, msg: "Not found" });
  ...
  updateaddress: function (req, res) {
    Address.findByIdAndUpdate(
      { _id: req.body.id },
      { title: req.body.title, address: req.body.address },
      function (err, address) {
        if (err) return res.send(500, { error: err });
        if (!address) {
          res.status(403).send({ success: false, msg: "Not found" });
  ...
  addseat: function (req, res) {
    if (!req.body.idtour || !req.body.floors1 || !req.body.floors2) {
      console.log(req.body);
      res.json({ success: false, msg: "Enter all fields" });
    } else {
      var newSeat = Seat({
  ...
  getseat: function (req, res) {
    var id = req.query.id;
    if (id) {
      Seat.find({ idcar: id }, function (err, seat) {
        if (err || !seat) {
          res.status(403).send({ success: false, msg: "Not found" });
        } else {
  ...
  dashboard: function (req, res) {
    Order.find({ status: "complete" }, function (err, order) {
      if (err || !order) {
        res.status(403).send({ success: false, msg: "Not found" });
      } else {
